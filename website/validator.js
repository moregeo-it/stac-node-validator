...
class STACValidator {
  ...
  showResults(results) {
    ...
    if (results.valid) {
      ...
      headerTitle.innerHTML = '<i class="fas fa-triangle-exclamation me-2"></i>Checks FAILED';
      statusDiv.innerHTML = ''; // Remove badge
    }
      ...
      results.errors.forEach((error) => {
        html += `
                    <div class="validation-error mb-3 p-3 border-start border-3 bg-light" style="border-color: #dc3545 !important;">
                        <h6 style="color: #dc3545;">${error.field || 'Unknown field'}</h6>
                        <p class="mb-1">${error.message}</p>
                        ${error.schema ? `<small class="text-muted">Schema: ${error.schema}</small>` : ''}
                    </div>
                `;
      });
    }
  ...
  showError(message) {
    ...
    headerDiv.classList.remove('validation-success');
    headerDiv.classList.add('validation-error');
    headerTitle.innerHTML = '<i class="fas fa-clipboard-check me-2"></i>Validation Results - Error';
    statusDiv.innerHTML = ''; // Remove badge
